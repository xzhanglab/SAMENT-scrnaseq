...
def load_data():
    ...
    file_path = os.path.join(dir_path, 'CD8_LysM-vs-ctrl_GSVA.csv')
...
def update_plot(keywords=[], logic='AND', width='100%', height=800, interactive=True):
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            font=dict(size=10),
            x=1.05,  # Move the legend slightly outside the plot
            y=1,
            traceorder='normal',
            bgcolor='rgba(255, 255, 255, 0.5)',  # Set background with opacity
            bordercolor='Black',
            borderwidth=1
        )
    )
...
if df is not None:
    ...
    fig_width = st.sidebar.slider('Figure Width', min_value=400, max_value=1600, value=1200, step=50)
    fig_height = st.sidebar.slider('Figure Height', min_value=400, max_value=1200, value=1000, step=50)
